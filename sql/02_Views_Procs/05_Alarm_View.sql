...
LEFT JOIN Substation s
    ON a.RelatedDeviceType = N'配电房' AND a.RelatedDeviceId = s.SubstationId
WHERE a.AlarmLevel = N'高'
GROUP BY s.SubstationId, s.SubstationCode, s.Name;
GO
--运维人员/管理员首页待办列表 + 告警总览仪表盘
CREATE VIEW V_Alarm_Unhandled_WithDeviceInfo
AS
SELECT
    a.AlarmId,
    a.AlarmType,
    ...
    CASE
        WHEN a.AlarmLevel = N'高' AND DATEDIFF(MINUTE, a.OccurTime, ISNULL(w.ResponseTime, SYSDATETIME())) > 15
            THEN N'响应超时'
...
LEFT JOIN EquipmentAsset ea
    ON a.RelatedDeviceType = ea.RelatedDeviceType
   AND a.RelatedDeviceId = ea.RelatedDeviceId
...
--质保到期前30天自动提醒（业务明确要求）
CREATE VIEW V_Equipment_WarrantyExpiry_Soon
AS
SELECT
    AssetId,
    AssetName,
...
WHERE ScrapStatus = N'正常使用'
  ...
  AND DATEADD(YEAR, WarrantyYears, InstallTime) <= DATEADD(DAY, 90, SYSDATETIME()); -- 提前90天开始关注
GO